...
- Explicit `$agent-academic-squad` invocation is the user's override and activates the skill. Treat a message beginning with `小分队` as the same explicit invocation whether it continues with a colon, comma, space, or task wording such as `小分队帮我...`; also accept an imperative such as `这个交给小分队`. Do not activate for negations such as `不用小分队`, `不要交给小分队`, or discussion-only mentions. Otherwise, delegation, planning, execution, review, or model-assignment wording activates it only after the academic gate passes.
