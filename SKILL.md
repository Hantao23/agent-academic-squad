...
- Treat every other automatic save as temporary. Run `python3 scripts/artifact_cache.py allocate --workspace-root "<workspace-root>" --kind <plan|review|handoff> --slug "<task-slug>"` from this skill directory and use the returned absolute path. Store it under `<workspace-root>/.tmp/agent-academic-squad/<plans|reviews|handoffs>/<YYYY-MM>/<DDTHHMMSSZ>-<task-slug>.md`, retain it for 30 days, and lazily remove only expired regular files created under this naming convention. Create only the selected kind and current month directories; do not create empty sibling directories.
