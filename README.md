...
- Substantial plans are saved automatically as dispatcher-generated auxiliary Markdown, but neither the file nor chat is forced into a universal template.
...
## Project artifacts and plan cache

Executing a task may produce source code, data or experiment outputs, logs, model weights, figures, papers, reports, presentations, review or search deliverables, and outputs from external skills. These are project artifacts: they remain under project ownership in the user-authorized project location and are returned or referenced there.

The squad's automatic cache has a much narrower purpose. It may persist only auxiliary text generated by the dispatcher, currently mainly a substantial long plan. It must not copy project artifacts into the plan cache or mis-save them as plans. This boundary does not enable automatic saving for every task output, and it does not introduce a fixed artifact or plan template.

When the squad produces a substantial plan, it saves that auxiliary plan text automatically for both implicit and explicit activation. The user can opt out by saying not to save it.
