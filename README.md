...
- Substantial plans, review reports, and handoff notes are saved automatically as dispatcher-generated auxiliary Markdown, but neither the file nor chat is forced into a universal template.
...
The squad's automatic cache has a much narrower purpose. It may persist only substantial plans, review reports, and handoff notes generated by the dispatcher for communication or continuation. If the user requested one as a project deliverable or authorized a project destination, it remains project-owned instead. The cache never duplicates project artifacts and does not introduce a fixed template.

The squad saves one of these auxiliary artifacts automatically only when enough detail would be lost in a chat-only response. Short plans, simple reviews, and ordinary answers stay in chat. The same rule applies to implicit and explicit activation, and the user can opt out.
