...
- Reusable plans, review reports, and handoff notes are saved automatically as dispatcher-generated auxiliary Markdown, but neither the file nor chat is forced into a universal template.
...
## Project artifacts and auxiliary cache

Executing a task may produce source code, data or experiment outputs, logs, model weights, figures, papers, presentations, reports or review deliverables saved as part of the project, and outputs from external skills. These are project artifacts: they remain under project ownership in the user-authorized project location and are returned or referenced there.

The squad's automatic cache has a much narrower purpose. It may persist only reusable plans, review reports, and handoff notes generated by the dispatcher for communication or continuation. If the user requested one as a project deliverable or authorized a project destination, it remains project-owned instead. The cache never duplicates project artifacts and does not introduce a fixed template.

The squad saves one when it has clear value for later execution, item-by-item verification, or reliable continuation and is more than a transient short reply. It need not be too long for chat, and there is no fixed word, section, or finding threshold. Short plans, simple reviews, and ordinary answers stay in chat; close cases prefer temporary storage unless sensitivity or project ownership argues against it. The same rule applies to implicit and explicit activation, and the user can opt out.

Automatic saving has two modes:

- Without an explicit request for permanent retention, the auxiliary file goes into a managed cache with a default 30-day lifetime.
- If the user asks to save or keep it permanently, or supplies a path, it is stored durably.
...
When a final answer exists only in conversation or tool output, the dispatcher streams its exact bytes to `persist_final.py` through standard input. It does not create fixed-name `*-source.md`, `*_final.md`, hash snapshots, or other staging files in the workspace merely to save the managed copy. An independently existing source file may be read in place but remains caller-owned. Disposable computation scratch belongs in an auto-cleaning system temporary directory or another user-authorized experiment location, and the dispatcher verifies before completion that none of its own staging files remain outside the managed layout.
...
A saved auxiliary file has no mandatory headings or section order. Plans retain material branches and execution detail, reviews retain consequential findings and evidence, and handoffs retain enough context to resume reliably. Inapplicable sections are omitted rather than filled with placeholders.

The saved file supplements the chat without forcing duplication. The chat still gives a proportional account of the mainline or findings, decisions that actually require the user, material caveats, current status, and the absolute file path. Details can remain in the file when repeating them would only add tokens.
...
Real E2E calls a model, can consume API quota, runs each case in an isolated temporary workspace, and writes results under `evals/results/`. Authentication, network, and timeout failures are reported separately from Skill behavior; missing required evidence is `inconclusive`. Use the optional manifests only when their external skills are installed, passing `--external-skill-root` to the installation root.
...
├── scripts/artifact_cache.py         # Managed auxiliary allocation and cleanup
